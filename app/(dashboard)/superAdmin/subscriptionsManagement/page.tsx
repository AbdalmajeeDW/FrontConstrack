...
export default function SubscriptionsPage() {
  ...
  const fetchSubscriptionsData = async () => {
    ...
    try {
      ...
      // Mock data - only Basic and Premium
  ...
  const statsCards = useMemo(() => {
    ...
    const premiumCount = subscriptions.filter((s) => s.subscriptionType === "premium").length;
    const basicCount = subscriptions.filter((s) => s.subscriptionType === "basic").length;
    const expiringSoon = subscriptions.filter(
      (s) => s.status === "active" && s.daysRemaining <= 7 && s.daysRemaining > 0
    ).length;
  ...
  const getStatusBadge = (status: SubscriptionStatus) => {
    const config = {
      active: { icon: <CheckCircle className="w-3 h-3" />, text: "Active", className: "bg-green-100 text-green-700" },
      expired: { icon: <AlertCircle className="w-3 h-3" />, text: "Expired", className: "bg-red-100 text-red-700" },
      suspended: { icon: <PauseCircle className="w-3 h-3" />, text: "Suspended", className: "bg-yellow-100 text-yellow-700" },
      cancelled: { icon: <XCircle className="w-3 h-3" />, text: "Cancelled", className: "bg-gray-100 text-gray-700" },
    };
        ...
        <div className=" mx-auto space-y-8">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-800 flex items-center gap-2 mb-4">
              ...
              Subscription Plans
              <span className="text-sm text-gray-500 font-normal">(Click to filter)</span>
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Basic Plan Card */}
              <motion.div
                whileHover={{ scale: 1.02 }}
                onClick={() => setFilterType(filterType === "basic" ? "all" : "basic")}
              ...
              >
                <div className="bg-gradient-to-r from-blue-500 to-cyan-600 p-6 text-white">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-3xl font-bold">150 SAR</span>
                  </div>
                ...
                <div className="bg-white p-4 flex justify-between items-center">
                  <div className="flex items-center gap-4">
                    <span className="text-gray-600 flex items-center gap-1">
                      ...
                      {subscriptions.filter((s) => s.subscriptionType === "basic").length} Companies
                    </span>
                    <span className="text-gray-600 flex items-center gap-1">
                      <DollarSign className="w-4 h-4" />
                      {subscriptions
                        .filter((s) => s.subscriptionType === "basic")
                        .reduce((sum, s) => sum + s.amountPaid, 0)} SAR
                    </span>
                  </div>
              ...
              <motion.div
                whileHover={{ scale: 1.02 }}
                onClick={() => setFilterType(filterType === "premium" ? "all" : "premium")}
                className={`cursor-pointer rounded-2xl overflow-hidden transition-all duration-300 ${
                  filterType === "premium"
              ...
              >
                <div className="bg-gradient-to-r from-yellow-500 to-amber-600 p-6 text-white">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-3xl font-bold">300 SAR</span>
                  </div>
                  <h4 className="text-2xl font-bold mt-4">Premium Plan</h4>
                  <p className="text-white/80 text-sm mt-1">30 days + All benefits</p>
                </div>
                <div className="bg-white p-4 flex justify-between items-center">
                  <div className="flex items-center gap-4">
                    <span className="text-gray-600 flex items-center gap-1">
                      <Users className="w-4 h-4" />
                      {subscriptions.filter((s) => s.subscriptionType === "premium").length} Companies
                    </span>
                    <span className="text-gray-600 flex items-center gap-1">
                      <DollarSign className="w-4 h-4" />
                      {subscriptions
                ...
                <table className="w-full min-w-[800px]">
                  <thead>
                    <tr className="border-b border-gray-200">
                      <th className="text-start py-3 px-4 text-gray-600 font-semibold">Company</th>
                      <th className="text-start py-3 px-4 text-gray-600 font-semibold">Plan</th>
                      <th className="text-start py-3 px-4 text-gray-600 font-semibold">Start Date</th>
                      <th className="text-start py-3 px-4 text-gray-600 font-semibold">End Date</th>
                      <th className="text-start py-3 px-4 text-gray-600 font-semibold">Remaining</th>
                      <th className="text-start py-3 px-4 text-gray-600 font-semibold">Amount</th>
                      <th className="text-start py-3 px-4 text-gray-600 font-semibold">Status</th>
                      <th className="text-start py-3 px-4 text-gray-600 font-semibold">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                        ...
                        <tr key={sub.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                          <td className="py-3 px-4">
                            <div className="flex items-center  gap-2">
                              ...
                              <span className="font-medium text-gray-800">{sub.playerName}</span>
                            </div>
                          </td>
                          <td className="py-3 px-4">
                            <span
                            ...
                            >
                              ...
                              {sub.subscriptionType === "premium" ? "Premium" : "Basic"}
                            </span>
                          </td>
                          <td className="py-3 px-4 text-gray-600">{sub.startDate}</td>
                          <td className="py-3 px-4 text-gray-600">{sub.endDate}</td>
                          <td className="py-3 px-4">
                            <span
                            ...
                            >
                              {sub.daysRemaining <= 0 ? "Expired" : `${sub.daysRemaining} days`}
                            </span>
                          </td>
                          <td className="py-3 px-4 font-medium text-gray-800">{sub.amountPaid} SAR</td>
                          <td className="py-3 px-4">
                            <span
