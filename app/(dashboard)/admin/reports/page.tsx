...
const mockReports: Report[] = [
  {
    id: 1,
    name: "Monthly Project Progress Summary",
    category: "project",
    description: "Complete overview of all project progress and milestones for June 2026",
    generatedAt: "2026-06-28T10:30:00",
    status: "ready",
    size: "2.4 MB",
    views: 45,
    downloads: 23,
    generatedBy: "Ahmed Al-Mansoori",
  },
  {
    id: 2,
    name: "Q2 Financial Analysis",
    category: "financial",
    description: "Budget analysis, cost breakdown, and financial forecasts for Q2",
    generatedAt: "2026-06-27T14:15:00",
    status: "ready",
    size: "1.8 MB",
    views: 38,
    downloads: 19,
    generatedBy: "Fatima Al-Hassan",
  },
  {
    id: 3,
    name: "Team Performance Report",
    category: "hr",
    description: "Employee productivity, task completion rates, and team efficiency",
    generatedAt: "2026-06-26T09:45:00",
    status: "processing",
    size: "3.1 MB",
    views: 12,
    downloads: 5,
    generatedBy: "Mohammed Al-Hamadi",
  },
  {
    id: 4,
    name: "Quality & Safety Audit",
    category: "quality",
    description: "Safety incidents, quality metrics, and compliance report",
    generatedAt: "2026-06-25T16:20:00",
    status: "ready",
    size: "4.2 MB",
    views: 28,
    downloads: 14,
    generatedBy: "Saeed Al-Kaabi",
  },
  {
    id: 5,
    name: "Resource Allocation Report",
    category: "resource",
    description: "Distribution of resources, equipment usage, and material tracking",
    generatedAt: "2026-06-24T11:00:00",
    status: "draft",
    size: "1.2 MB",
    views: 8,
    downloads: 2,
    generatedBy: "Nadia Al-Shamsi",
  },
  {
    id: 6,
    name: "Project Timeline Analysis",
    category: "project",
    description: "Schedule performance, delays, and timeline predictions",
    generatedAt: "2026-06-23T13:30:00",
    status: "ready",
    size: "3.7 MB",
    views: 34,
    downloads: 17,
    generatedBy: "Khalid Al-Mazrouei",
  },
  {
    id: 7,
    name: "Budget Variance Report",
    category: "financial",
    description: "Analysis of budget variances across all active projects",
...
const categoryConfig = {
  project: { label: "Project", color: "bg-purple-100 text-purple-700", icon: Building2 },
  financial: { label: "Financial", color: "bg-emerald-100 text-emerald-700", icon: DollarSign },
  hr: { label: "HR", color: "bg-blue-100 text-blue-700", icon: Users },
  quality: { label: "Quality", color: "bg-amber-100 text-amber-700", icon: Award },
  resource: { label: "Resource", color: "bg-rose-100 text-rose-700", icon: HardDrive },
};
            ...
            {metrics.map((metric, index) => (
              ...
              >
                ...
                <div className="relative bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden">
                  <div className={`absolute top-0 right-0 w-32 h-32 bg-linear-to-br ${metric.color} opacity-10 rounded-full blur-2xl group-hover:opacity-20 transition-opacity`}></div>
                  <div className="relative p-6">
                    <div className="flex items-center justify-between mb-4">
                      <div className={`p-3 rounded-xl ${metric.color.replace("from-", "bg-").replace(" to-", "-")} bg-opacity-10`}>
                        <metric.icon className="w-6 h-6" />
                      </div>
                      <span className={`text-sm font-medium ${metric.change >= 0 ? 'text-emerald-600 bg-emerald-100' : 'text-rose-600 bg-rose-100'} px-2 py-1 rounded-full`}>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Generate reports or export data instantly</p>
                </div>
              </div>
              <div className="flex gap-3 flex-wrap">
                ...
                <tbody className="divide-y divide-slate-100">
                  {paginatedReports.length === 0 ? (
                    ...
                    paginatedReports.map((report, idx) => {
                      ...
                      const categoryColor = categoryConfig[report.category].color;
      ...
      <p className="truncate font-medium text-slate-900 group-hover:text-purple-600 transition-colors text-sm">
        {report.name}
                          ...
                          <td className="px-4 py-3 text-center">
                            <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${categoryColor}`}>
                          ...
                          <td className="hidden md:table-cell px-4 py-3">
                            <div className="text-center">
                              <p className="text-sm text-slate-700">{formatDate(report.generatedAt)}</p>
                              <p className="text-xs text-slate-400">{formatTime(report.generatedAt)}</p>
                            </div>
                          </td>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center justify-center gap-3">
                              <button className="p-1.5 hover:bg-slate-100 rounded-lg transition-colors" title="View Report">
          ...
          >
            <div className="bg-white rounded-2xl shadow-lg p-6 col-span-1">
              ...
              <div className="space-y-3">
                {Object.entries(categoryConfig).map(([key, value]) => {
                  const count = mockReports.filter(r => r.category === key).length;
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 col-span-1">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 bg-purple-50 rounded-lg">
                  ...
                  <span className="text-sm font-semibold text-purple-600">Project Reports</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-emerald-50 rounded-lg">
                  <span className="text-sm text-gray-600">Avg. Views</span>
                  <span className="text-sm font-semibold text-emerald-600">
                    {Math.round(mockReports.reduce((sum, r) => sum + r.views, 0) / mockReports.length)}
                  </span>
                </div>
                <div className="flex items-center justify-between p-3 bg-amber-50 rounded-lg">
                  <span className="text-sm text-gray-600">Total Downloads</span>
                  <span className="text-sm font-semibold text-amber-600">{stats.totalDownloads}</span>
                </div>
              </div>
            </div>
          </motion.div>
